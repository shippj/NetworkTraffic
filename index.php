...
if (isset($_GET["day"])){
	...
	$sql = "select to_ip, round(sum(sum_size)/1000000000,1) as GB from reports where day=" . $_GET["day"] . " group by to_ip order by sum(sum_size) desc limit 10;";
...
elseif (isset($_GET["today"])){
	...
	$sql = "select to_ip, round(sum(size)/1000000000,1) as GB from data where day=" . $day . " group by to_ip order by sum(size) desc limit 10;";
	$result = $conn->query($sql);
...
elseif (isset($_GET["first"])){
	...
	$sql = "select to_ip, round(sum(sum_size)/1000000000,1) as GB from reports where day>=" . $_GET["first"] . " and day<=" . $_GET["last"] . " group by to_ip order by sum(sum_size) desc limit 10;";
	//echo $sql . "<br>";
	$result = $conn->query($sql);
	...
	echo "<a href=old.php>Switch to old version</a><br><br>";
